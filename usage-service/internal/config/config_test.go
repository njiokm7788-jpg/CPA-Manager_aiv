...
func TestLoadReadsConfigAndResolvesRelativePaths(t *testing.T) {
  ...
  "tlsSkipVerify": true
	...
	if cfg.DatabaseURL != "postgres://user:pass@host/db?sslmode=require" {
		t.Fatalf("DatabaseURL = %q", cfg.DatabaseURL)
	}
	if cfg.DBMaxOpenConns != 11 || cfg.DBMaxIdleConns != 6 || cfg.DBConnMaxLifetime != 45*time.Minute {
...
func clearConfigEnv(t *testing.T) {
	...
	for _, key := range []string{
		...
		"USAGE_DB_PATH",
		"USAGE_DATABASE_URL",
		"USAGE_DB_MAX_OPEN_CONNS",
		"USAGE_DB_MAX_IDLE_CONNS",
		"USAGE_DB_CONN_MAX_LIFETIME_MINUTES",
		"CPA_UPSTREAM_URL",
		"CPA_MANAGEMENT_KEY",
